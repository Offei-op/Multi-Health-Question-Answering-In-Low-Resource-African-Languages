...
FEATURES = OUT / "cluster_candidate_features_lite.csv"
IDCOL = "ID"
GCOL = "subset"
RANDOM_STATE = 13
...
def load_fast_frame() -> pd.DataFrame:
    ...
    df["_fast_pool_score"] = (
        ...
        + 0.6 * df["query_question_jaccard"].fillna(0)
        + 0.4 * df["query_answer_jaccard"].fillna(0)
    )
    df = df.sort_values([IDCOL, "_fast_pool_score"], ascending=[True, False])
    df = df.groupby(IDCOL, sort=False).head(80).reset_index(drop=True)
    return df
...
def baseline_rows(df: pd.DataFrame) -> list[dict]:
    ...
    for score_col in ["_fast_pool_score", "rrf", "source_count", "inv_bge200_q2q_best_rank"]:
    ...
    rows.append({"model": "fast_candidate_pool_oracle", "score": float(oracle["target_r1"].mean()), "rmse": 0.0, "chosen_rows": len(oracle)})
    return rows


def run() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    df = load_fast_frame()
    df.to_csv(OUT / "cluster_candidate_features_fast.csv", index=False)
    ...
    models = {
        "hist_gb_fast_depth6": HistGradientBoostingRegressor(
            max_iter=260,
            learning_rate=0.045,
            max_leaf_nodes=31,
        ...
        "hist_gb_fast_depth4": HistGradientBoostingRegressor(
            max_iter=220,
            learning_rate=0.05,
            max_leaf_nodes=15,
...
def main() -> None:
    ...
    leader.to_csv(OUT / "cluster_selector_fast_leaderboard.csv", index=False)
    choices.to_csv(OUT / "cluster_selector_fast_oof_choices.csv", index=False)
    subset.to_csv(OUT / "cluster_selector_fast_subset_summary.csv", index=False)
    summary = {"best": leader.iloc[0].to_dict(), "leaderboard": leader.to_dict(orient="records")}
    (OUT / "cluster_selector_fast_summary.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
